...
class VarianteFilter(django_filters.FilterSet):
    ...
    # Método específico para filtrar coluna "Filtro", em que o NULL na verdade pode ser "PASS"
...
class AmostraFilter(django_filters.FilterSet):
    ...
    amostra__notnull = django_filters.BooleanFilter(
        field_name='amostra', 
        method='filter_not_null', 
        widget=forms.CheckboxInput(attrs={'class': 'hidden-checkbox'})
    )
    
    amostra__exact = django_filters.BooleanFilter(
        field_name='amostra', 
        method='filter_exact_toggle', 
        widget=forms.CheckboxInput(attrs={'class': 'hidden-checkbox'})
    )

    gt__notnull = django_filters.BooleanFilter(
        field_name='gt', 
        method='filter_not_null', 
        widget=forms.CheckboxInput(attrs={'class': 'hidden-checkbox'})
    )
    gt__exact = django_filters.BooleanFilter(
        field_name='gt', 
        method='filter_exact_toggle', 
        widget=forms.CheckboxInput(attrs={'class': 'hidden-checkbox'})
    )
    ...
    class Meta:
        model = FltrdCybersegChr21Amostras
        fields =  ['id_variante', 'amostra', 'gt', 'af', 'dp']
